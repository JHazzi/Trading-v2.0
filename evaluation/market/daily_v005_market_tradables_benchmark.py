...
def daily_ic(frame, target_col, pred_col):
    ...
    def agg(x):
        return {
            "days": len(x),
            "mean": None if not x else float(np.mean(x)),
...
def moving_block_bootstrap_days(
    ...
    daily = (
        x.groupby("origin_trading_day", sort=True)["delta"]
        .mean()
        .to_numpy(float)
    )
    n = len(daily)
    if n <= block_length:
        raise ValueError("insufficient origin days for moving block bootstrap")
...
def masks(frame, boundary):
    ...
    if not train.any() or not test.any():
        raise RuntimeError("empty V005 fold")
    latest = str(frame.loc[train, "target_end_day"].astype(str).max())
    if latest >= first:
        raise AssertionError(
            f"target leakage: latest train target {latest} >= {first}"
        )
    return train, test
