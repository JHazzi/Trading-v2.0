"""Dependence-aware evaluation and immutable gates for Temporal V002.
...
def development_gate(summary: Mapping[str, Any], rules: Mapping[str, Any]) -> dict[str, Any]:
    ...
    margin = float(rules["no_harm_margin_pct"])
    per_anchor = summary["per_anchor_point_delta_pct"]
    no_harm = {
        str(int(tau)): float(per_anchor[str(int(tau))]) >= -margin
        for tau in rules["no_harm_horizons"]
    }
    checks = {
        "primary_interval_lower_bound_positive": primary_ci[0] > 0.0,
        "minimum_positive_anchors_met": int(summary["positive_anchors"]) >= int(rules["minimum_positive_development_anchors"]),
        "minimum_positive_folds_met": int(summary["positive_folds"]) >= int(rules["minimum_positive_outer_folds"]),
        ...
        "no_harm_at_every_frozen_long_horizon": all(no_harm.values()),
    }
    ...
    if all(required):
        ...
        status = "FAIL_CLOSE_TEMPORAL_DISTRIBUTIONAL_V002_BRANCH"
    else:
        status = "INCONCLUSIVE_OR_AUXILIARY_GATE_FAIL_NO_HOLDOUT_OPEN"
    return {
        "status": status, "checks": checks, "no_harm_by_horizon": no_harm,
        "no_harm_margin_pct": margin, "primary_ci95": primary_ci,
...
def holdout_gate(summary: Mapping[str, Any], rules: Mapping[str, Any]) -> dict[str, Any]:
    ...
    margin = float(rules["no_harm_margin_pct"])
    per_horizon = summary["per_holdout_horizon_point_delta_pct"]
    no_harm = {
        str(int(tau)): float(per_horizon[str(int(tau))]) >= -margin
        for tau in rules["no_harm_holdout_horizons"]
    }
    checks = {
        "primary_interval_lower_bound_positive": ci[0] > 0.0,
        "minimum_positive_holdout_horizons_met": int(summary["positive_holdout_horizons"]) >= int(rules["minimum_positive_holdout_taus"]),
        "minimum_positive_folds_met": int(summary["positive_folds"]) >= int(rules["minimum_positive_outer_folds"]),
        "minimum_improved_quantiles_met": int(summary["improved_quantiles"]) >= int(rules["minimum_improved_quantiles"]),
        "candidate_calibration_not_worse": float(summary["candidate_calibration_mae"]) <= float(summary["reference_calibration_mae"]),
        "no_harm_at_every_frozen_long_holdout": all(no_harm.values()),
    }
    return {
        "status": "PASS_TEMPORAL_DISTRIBUTIONAL_V002_HOLDOUT" if all(checks.values()) else "FAIL_CLOSE_TEMPORAL_DISTRIBUTIONAL_V002_BRANCH",
        "checks": checks,
        "no_harm_by_horizon": no_harm,
        "no_harm_margin_pct": margin,
        "primary_ci95": ci,
