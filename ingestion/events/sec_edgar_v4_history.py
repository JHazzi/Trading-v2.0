...
INGESTION_VERSION = "sec_metadata_v0.4.0_history_alias"
...
def parse_issuer_target(value: str) -> dict[str, str | None]:
    """
    Parse explicit issuer continuity target:
        TICKER=CIK
    Example:
        XOM=34088
    """
    ...
    if "=" not in raw:
        raise ValueError(
            f"Issuer target inválido {value!r}; esperado TICKER=CIK"
        )
    ticker, cik = (part.strip() for part in raw.split("=", 1))
    if not ticker:
        raise ValueError("Issuer target sin ticker")
    if not cik:
        raise ValueError("Issuer target sin CIK")
    return {
        "ticker": ticker.upper(),
        "cik": normalize_cik(cik),
        "title": None,
        "target_resolution": "explicit_ticker_cik",
    }
    ...
    *,
    ...
    for ticker in tickers:
        ...
        if rec is None:
            ...
            targets.append({
                **rec,
                "target_resolution": "sec_current_ticker_mapping",
            })

    for issuer in issuers:
        targets.append(parse_issuer_target(issuer))
    ...
    for cik in ciks:
        targets.append({
            ...
            "cik": normalize_cik(cik),
            "title": None,
            "target_resolution": "explicit_cik_unlinked",
        })
    ...
    for target in targets:
        key = (
            None if target.get("ticker") is None
            else str(target["ticker"]).upper(),
            normalize_cik(str(target["cik"])),
        )
        if key in seen:
            continue
        seen.add(key)
...
def persist_filing_observation(conn, store, *, run_id, parent_raw_id, retrieval_id,
    ...
    # Legacy-safe migration-016 resolution.
    # Identical metadata content can already exist under a migrated immutable
    # version/raw pair. INSERT OR IGNORE then preserves that row, so the
    # observation must reference the pair that actually exists.
...
def run_ingestion(*, db, raw_root, tickers, ciks, issuers, forms, max_filings,
...
def main():
    ...
    p.add_argument("--issuer",action="append",default=[], help="Explicit issuer continuity target TICKER=CIK")
    ...
    a=p.parse_args()
    if not a.ticker and not a.cik and not a.issuer: raise SystemExit("Indicá --ticker, --issuer o --cik")
    ua=a.user_agent or os.environ.get("SEC_USER_AGENT")
    result=run_ingestion(db=a.db,raw_root=a.raw_root,tickers=a.ticker,ciks=a.cik,issuers=a.issuer,
