"""Frozen, resumable residual runner for Q(total shareholder return | X, tau).
...
from evaluation.market.temporal_distributional_v002 import (  # noqa: E402
    calibration_counts,
    development_gate,
    equal_tau_day_table,
...
from models.market.temporal_distributional_v002 import (  # noqa: E402
...
DEFAULT_CONFIG = ROOT / "config" / "temporal_distributional_runner_v002.json"
DEFAULT_PROTOCOL = ROOT / "config" / "temporal_distributional_preregistration_v002.json"
DEFAULT_FOLDS = ROOT / "reports" / "temporal_distributional_v002" / "fold_plan.json"
DEFAULT_PREREG = ROOT / "reports" / "temporal_distributional_v002" / "preregistration_plan.json"
...
DEFAULT_OUTPUT = ROOT / "reports" / "temporal_distributional_runner_v002"
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if runner.get("version") != "market_temporal_distributional_runner_v002":
        raise ValueError("unsupported_temporal_runner_contract")
    reference = runner.get("canonical_preregistration") or {}
    protocol_path = ROOT / str(reference.get("path", ""))
    if protocol_path.resolve() != DEFAULT_PROTOCOL.resolve() or not protocol_path.exists():
        raise ValueError("canonical_preregistration_path_mismatch")
    if digest(protocol_path) != reference.get("sha256"):
        raise ValueError("canonical_preregistration_hash_mismatch")
    protocol = load_json(protocol_path)
    if protocol.get("version") != reference.get("version"):
        raise ValueError("canonical_preregistration_version_mismatch")
    horizon = protocol["horizon_contract"]
    model = protocol["model_contract"]
    anchors = list(map(int, horizon["development_training_anchors"]))
    holdouts = list(map(int, horizon["sealed_generalization_holdouts"]))
    if len(anchors) != 12 or len(holdouts) != 5 or set(anchors) & set(holdouts):
        raise ValueError("invalid_frozen_horizon_partition")
    if sorted(anchors + holdouts) != sorted(map(int, horizon["materialized_checkpoints"])):
        raise ValueError("materialized_horizon_partition_mismatch")
    panel = protocol["training_panel"]
    ...
    target = protocol["target_representation"]
    if target.get("outcome_clipping") != "none" or target.get("fit_domain") != "log_total_wealth":
        raise ValueError("target_clipping_is_forbidden")
    shrink = protocol["residual_regularization"]
    if shrink.get("formula") != "alpha(tau) = 2 ** (-(tau - 1) / 63)" or float(shrink.get("half_life_sessions", 0)) != 63.0:
        raise ValueError("residual_shrinkage_formula_mismatch")
    crossfit = protocol["internal_cross_fitting"]
    if crossfit.get("split_policy") != "causal_purged_expanding_origin_day_blocks" or crossfit.get("future_rows_allowed_in_internal_training") is not False:
        raise ValueError("internal_crossfit_is_not_causal")
    if int(runner["execution"].get("maximum_tau_sessions", 0)) != 252:
    ...
    return {
        ...
        "expected_v002_sha256": runner["expected_v002_sha256"],
        "quantiles": model["quantiles"],
        "development_anchors": anchors,
        "sealed_holdouts": holdouts,
        "target_representation": target,
        "training_panel": panel,
        "internal_cross_fitting": crossfit,
        "residual_regularization": shrink,
        "models": {
            "model_version": model["model_version"],
            "own_features": model["own_features"],
            "scale_feature": model["scale_feature"],
            "preserved_placebo_features": model["preserved_placebo_features"],
            "placebo_seeds": model["placebo_seeds"],
            "profile_base": model["profile_base"],
            "profile_residual": model["profile_residual"],
        },
        "evaluation": {
            **protocol["outer_evaluation"],
            **protocol["dependence_and_uncertainty"],
        },
        "development_gate": protocol["development_gate_before_holdout_open"],
        "holdout_gate": protocol["sealed_holdout_gate"],
...
def runner_plan(
    ...
    required = {
        ...
        "preregistration": (prereg_path, "READY_FOR_RUNNER_PREFLIGHT_NO_TRAINING"),
    }
    ...
    if prereg_path.exists() and load_json(prereg_path).get("config_sha256") != digest(cfg["protocol_path"]):
        failures.append("PREREGISTRATION_PLAN_NOT_BOUND_TO_CANONICAL_PROTOCOL")
    ...
    result = {
        ...
        "config_sha256": digest(config_path), "protocol_sha256": digest(cfg["protocol_path"]),
...
def _require_plan(output: Path, config_path: Path) -> dict[str, Any]:
    ...
    if plan.get("runner_sha256") != digest(Path(__file__)):
        raise RuntimeError("runner_code_changed_after_plan")
    cfg = load_config(config_path)
    if plan.get("protocol_sha256") != digest(cfg["protocol_path"]):
        raise RuntimeError("canonical_protocol_changed_after_plan")
    return plan
...
def _panel_query(features: Sequence[str], taus: Sequence[int], mode: str) -> str:
    ...
    return (
        "SELECT o.origin_id,t.origin_trading_day,o.tau_sessions,o.target_trading_day,o.total_return_pct," + columns + " "
...
def load_panel(
    ...
    target_day_parts: list[np.ndarray] = []
    ...
    for frame in pd.read_sql_query(query, conn, params=parameters, chunksize=int(chunk_rows)):
        ...
        target_day_parts.append(frame["target_trading_day"].astype(str).to_numpy())
    ...
    target_day = np.concatenate(target_day_parts)
    ...
    return Panel(
        origin_id, day_index.astype("int16"), unique_days.astype(str).tolist(),
        tau, target_day, actual, own_state,
    )
...
def _panel_weights(panel: Panel) -> np.ndarray:
    """Give each day total weight 1, then each origin and selected tau equally."""
    frame = pd.DataFrame({"day": panel.origin_day_index, "origin": panel.origin_id})
    origin_rows = frame.groupby(["day", "origin"], observed=True)["origin"].transform("size").to_numpy(dtype=float)
    origin_count = frame.groupby("day", observed=True)["origin"].transform("nunique").to_numpy(dtype=float)
    weights = 1.0 / (origin_count * origin_rows)
    return weights * (len(weights) / weights.sum())


def _fit_models(
    X: np.ndarray, y_log: np.ndarray, cfg: Mapping[str, Any], profile_name: str,
...
def _fit_residual_models(
    X: np.ndarray, residual_by_quantile: np.ndarray, cfg: Mapping[str, Any],
    sample_weight: np.ndarray | None = None,
) -> dict[str, Any]:
    profile = cfg["models"]["profile_residual"]
    quantiles = list(map(float, cfg["quantiles"]))
    target = np.asarray(residual_by_quantile, dtype=float)
    if target.shape != (len(X), len(quantiles)):
        raise ValueError("residual target matrix differs from quantile manifest")
    models: dict[str, Any] = {}
    for column, q in enumerate(quantiles):
        ...
        model.fit(X, target[:, column], sample_weight=sample_weight)
        models[str(q)] = model
    return models


def _predict_log_models(models: Mapping[str, Any], X: np.ndarray, quantiles: Sequence[float]) -> np.ndarray:
    return np.column_stack([models[str(float(q))].predict(X) for q in quantiles])


def _predict_models(models: Mapping[str, Any], X: np.ndarray, quantiles: Sequence[float]) -> np.ndarray:
    return fit_to_target(monotone_rearrange(_predict_log_models(models, X, quantiles))).astype("float32")


def _take_panel(panel: Panel, take: np.ndarray) -> Panel:
    mask = np.asarray(take, dtype=bool)
    raw_days = np.asarray(panel.origin_days, dtype=str)[panel.origin_day_index[mask]]
    unique_days, day_index = np.unique(raw_days, return_inverse=True)
    return Panel(
        panel.origin_id[mask], day_index.astype("int16"), unique_days.astype(str).tolist(),
        panel.tau_sessions[mask], panel.target_trading_day[mask], panel.actual_pct[mask],
        panel.own_state[mask],
    )


def _causal_oof_base(train: Panel, cfg: Mapping[str, Any]) -> tuple[np.ndarray, np.ndarray, list[dict[str, Any]]]:
    contract = cfg["internal_cross_fitting"]
    quantiles = list(map(float, cfg["quantiles"]))
    days = np.asarray(train.origin_days, dtype=str)
    initial = max(1, int(np.floor(len(days) * float(contract["initial_fraction_of_outer_training_origin_days"]))))
    remaining = len(days) - initial
    folds = int(contract["folds"])
    if remaining < folds:
        raise RuntimeError("not_enough_days_for_causal_internal_crossfit")
    ...
    for fold in range(folds):
        ...
        start = stop
    oof = np.full((len(train.actual_pct), len(quantiles)), np.nan, dtype=float)
    audit: list[dict[str, Any]] = []
    scale_index = list(cfg["models"]["own_features"]).index(cfg["models"]["scale_feature"])
    full_design = _design(train)
    base_design = np.column_stack((train.own_state[:, scale_index], full_design[:, -3:])).astype("float32")
    for number, (start, stop) in enumerate(bounds, 1):
        first_validation_day = str(days[start])
        fit_mask = (train.origin_day_index < start) & (train.target_trading_day < first_validation_day)
        validation_mask = (train.origin_day_index >= start) & (train.origin_day_index < stop)
        fit_panel = _take_panel(train, fit_mask)
        models = _fit_models(
            base_design[fit_mask], target_to_fit(train.actual_pct[fit_mask]), cfg,
            "profile_base", _panel_weights(fit_panel),
        )
        oof[validation_mask] = _predict_log_models(models, base_design[validation_mask], quantiles)
        latest_fit_target = max(map(str, train.target_trading_day[fit_mask]))
        if latest_fit_target >= first_validation_day:
            raise RuntimeError("internal_crossfit_target_day_purge_failed")
        audit.append({
            "fold": number, "first_validation_origin_day": first_validation_day,
            "last_validation_origin_day": str(days[stop - 1]),
            "training_rows": int(np.count_nonzero(fit_mask)),
            "validation_rows": int(np.count_nonzero(validation_mask)),
            "latest_training_target_day": latest_fit_target,
            "future_rows_used": False,
        })
    usable = np.isfinite(oof).all(axis=1)
    if int(np.count_nonzero(usable)) < int(contract["minimum_oof_rows"]):
        raise RuntimeError("causal_oof_rows_below_frozen_minimum")
    return usable, oof, audit
...
def _fold_report(
    ...
    return {
        "version": "market_temporal_distributional_fold_result_v002", "phase": phase,
...
def run_development_fold(
    ...
    X_train, X_test = _design(train), _design(test)
    y_log = target_to_fit(train.actual_pct)
    quantiles = list(map(float, cfg["quantiles"]))
    scale_index = features.index(cfg["models"]["scale_feature"])
    reference_train = np.column_stack((train.own_state[:, scale_index], X_train[:, -3:])).astype("float32")
    reference_test = np.column_stack((test.own_state[:, scale_index], X_test[:, -3:])).astype("float32")

    oof_mask, base_oof_log, crossfit_audit = _causal_oof_base(train, cfg)
    residual_panel = _take_panel(train, oof_mask)
    residual_y = residual_targets(y_log[oof_mask], base_oof_log[oof_mask])
    residual_weights = _panel_weights(residual_panel)
    residual_models = _fit_residual_models(X_train[oof_mask], residual_y, cfg, residual_weights)

    reference_models = _fit_models(
        reference_train, y_log, cfg, "profile_base", _panel_weights(train),
    )
    reference_log = _predict_log_models(reference_models, reference_test, quantiles)
    reference_pred = fit_to_target(monotone_rearrange(reference_log)).astype("float32")
    residual_test_log = _predict_log_models(residual_models, X_test, quantiles)
    candidate_log = combine_log_quantiles(
        reference_log, residual_test_log, test.tau_sessions,
        float(cfg["residual_regularization"]["half_life_sessions"]),
    )
    candidate_pred = fit_to_target(candidate_log).astype("float32")
    ...
    for seed in map(int, cfg["models"]["placebo_seeds"]):
        placebo_train, audit = _placebo_design(residual_panel, X_train[oof_mask], cfg, seed)
        models = _fit_residual_models(placebo_train, residual_y, cfg, residual_weights)
        placebo_log = _predict_log_models(models, X_test, quantiles)
        placebo_predictions[str(seed)] = fit_to_target(combine_log_quantiles(
            reference_log, placebo_log, test.tau_sessions,
            float(cfg["residual_regularization"]["half_life_sessions"]),
        )).astype("float32")
    ...
    candidate_bundle = {
        "version": "market_temporal_distributional_model_bundle_v002", "fold": int(fold),
        "model_version": cfg["models"]["model_version"],
        "quantiles": quantiles, "own_features": features,
        "scale_feature": cfg["models"]["scale_feature"],
        "base_models": reference_models, "residual_models": residual_models,
        "residual_regularization": cfg["residual_regularization"],
        "simple_train_only_log_quantiles": simple, "target_representation": cfg["target_representation"],
        "training_first_test_origin_day": definition["first_test_origin_day"],
        "config_sha256": digest(config_path), "protocol_sha256": digest(cfg["protocol_path"]),
    }
    reference_bundle = {
        "version": "market_temporal_distributional_reference_bundle_v002", "fold": int(fold),
    ...
    report = _fold_report(
        ...
        placebo_predictions, {
            ...
            "causal_oof_residual_rows": int(np.count_nonzero(oof_mask)),
            "causal_oof_burn_in_rows_excluded_from_residual": int(np.count_nonzero(~oof_mask)),
            "internal_crossfit": crossfit_audit,
            "sample_weight_formula": cfg["training_panel"]["sample_weight_formula"],
            "shrinkage_alpha": {
                str(tau): float(horizon_shrinkage([tau], cfg["residual_regularization"]["half_life_sessions"])[0])
                for tau in anchors
            },
...
def aggregate_development(config_path: Path, output: Path) -> dict[str, Any]:
    ...
    result = {
        "version": "market_temporal_distributional_development_summary_v002", "stage": "development_aggregate",
...
def freeze_development(config_path: Path, output: Path) -> dict[str, Any]:
    if (output / "development_closure.json").exists():
        raise RuntimeError("negative_branch_closure_blocks_freeze")
    ...
    paths = [
        config_path, DEFAULT_PROTOCOL, Path(__file__), ROOT / "models" / "market" / "temporal_distributional_v002.py",
        ROOT / "evaluation" / "market" / "temporal_distributional_v002.py", output / "plan.json", summary_path,
    ...
    result = {
        "version": "market_temporal_distributional_development_freeze_v002", "status": "PASS",
...
def _verify_freeze(output: Path) -> tuple[dict[str, Any], str]:
    if (output / "development_closure.json").exists():
        raise RuntimeError("negative_branch_closure_blocks_holdout")
...
def _open_marker(output: Path, freeze_sha: str, fold: int) -> dict[str, Any]:
    ...
    if path.exists():
        ...
        marker = {
            "version": "market_temporal_distributional_holdout_opening_v002", "status": "OPENING",
...
def run_holdout_fold(
    ...
    scale = features.index(cfg["models"]["scale_feature"])
    test_design = _design(test)
    reference_design = np.column_stack((test.own_state[:, scale], test_design[:, -3:])).astype("float32")
    reference = _predict_models(reference_bundle["models"], reference_design, cfg["quantiles"])
...
def aggregate_holdout(config_path: Path, output: Path) -> dict[str, Any]:
    ...
    result = {
        "version": "market_temporal_distributional_holdout_summary_v002", "stage": "holdout_aggregate",
...
def close_branch(config_path: Path, output: Path) -> dict[str, Any]:
    ...
    summary = load_json(summary_path)
    terminal = str(summary.get("status"))
    if terminal not in {
        "FAIL_CLOSE_TEMPORAL_DISTRIBUTIONAL_V002_BRANCH",
        "INCONCLUSIVE_OR_AUXILIARY_GATE_FAIL_NO_HOLDOUT_OPEN",
    }:
        raise RuntimeError("branch_closure_requires_explicit_nonpass_state")
    ...
    for fold in range(1, 6):
        ...
        if not report_path.exists():
            raise RuntimeError(f"closure_fold_report_missing:{fold}")
    ...
    manifest = {str(path.relative_to(ROOT)): digest(path) for path in sorted(set(paths))}
    result = {
        "version": "market_temporal_distributional_development_closure_v002",
        "status": "CLOSED_NEGATIVE" if terminal.startswith("FAIL_") else "CLOSED_INCONCLUSIVE",
        "closed_at": utc_now(),
        ...
        "next_gate": "DO_NOT_OPEN_HOLDOUTS_END_OF_V002",
    }
...
def status_report(output: Path) -> dict[str, Any]:
    ...
    return {
        "version": "market_temporal_distributional_runner_status_v002", "plan": status(output / "plan.json"),
